...
public class PostThumbnailTask {
	...
	public void deleteUnusedFiles() {
		...
		List<Path> fileListPaths = fileList.stream()
			.map(vo -> Paths.get(ROOT_PATH, vo.getPath(), vo.getUuid() + NAME_SEPARATOR + vo.getName()))
			.collect(Collectors.toList());

		fileList.stream()
			.map(
				vo -> Paths.get(ROOT_PATH, vo.getPath(), THUMBNAIL_MARK + vo.getUuid() + NAME_SEPARATOR + vo.getName()))
